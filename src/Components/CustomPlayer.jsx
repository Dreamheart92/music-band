...
export default function CustomPlayer({trackUrl, artist, trackName, type}) {
    ...
    let albumStyle = 'flex w-full h-fit justify-between cursor-pointer items-center hover:bg-zinc-900 p-3';

    if (currentSong === trackName) {
        albumStyle += ' bg-zinc-900'
    }
    ...
    return (
        <section
            ...
            {type === 'album' &&
                <>
                    <div className='w-full px-2 h-full items-start flex flex-col'>
                        <h1 className={`font-mono text-zinc-200 text-[.95em]`}>{trackName}</h1>
                        {type === 'album' &&
